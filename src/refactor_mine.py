    ...
    def _generate(self):
        ...
        if self.output_in_one_file_var.get() == "on":
            ...
            for batch_start in range(0, total, batch_size):
                ...
                batch_files = chapter_files[batch_start:batch_end]
                for index, chapter_file in enumerate(batch_files, start=batch_start + 1):
                    chapter_text = str(self.chapter_text_by_filename[chapter_file]).strip()
                    self._generate_one_mp3_file(chapter_text, file_name, index, total)
    ...
    def _play_sample(self):
        ...
        try:
            ...
            try:
                ...
                raise RuntimeError("ffplay (FFmpeg) is not installed or not found in PATH.")

            # Play the audio
            subprocess.run(
                ["ffplay", "-nodisp", "-autoexit", sample_mp3_path],
