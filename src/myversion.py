...
class EpubAudioConverterUI(tk.Tk):
    ...
    def _generate(self):
        ...
        if self.output_in_one_file_var.get() == "on":
            ...
            total = len(self.chapter_listbox.curselection())
            i = 0
            for idx in self.chapter_listbox.curselection():
                i += 1
                chapter_file = self.chapter_listbox.get(idx)  # Corrected line
    ...
    def _on_epub_file_path_change(self):
        self.load_epub_file_chapters()

    def _delete_chapter_list(self):
        if self.chapter_listbox.size() > 0:
            self.chapter_text_by_filename.clear()
            self.chapter_text_by_filename = {}
