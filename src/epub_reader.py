...
def get_title_and_chapters(epub_file_path):
    ...
    # First try by filtering file names
    chapters_filename_and_text = [
        (x.file_name, _get_chapter_text(x))
        for x in book.get_items_of_type(ebooklib.ITEM_DOCUMENT)
        if type(x) is ebooklib.epub.EpubHtml and x.media_type == "application/xhtml+xml"
    ]

    chapters_filename_and_text = [
        x for x in chapters_filename_and_text if x[1].replace("\n", "") != ""
    ]
